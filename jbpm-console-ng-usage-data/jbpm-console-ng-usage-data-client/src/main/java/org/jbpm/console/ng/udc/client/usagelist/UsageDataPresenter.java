...
import org.jbpm.console.ng.udc.client.util.UsageEventSummaryBuilder;
...
public class UsageDataPresenter {
	...
	public void refreshUsageDataCollector() {
		usageDataService.call(new RemoteCallback<Queue<UsageEventSummary>>() {
			...
			public void callback(Queue<UsageEventSummary> events) {
				...
				EventTypes.valueOf(view.getEventTypesList().getValue()),
				identity.getName());
	}

	public void clearUsageData() {
		if (allUsageEventSummaries != null && !allUsageEventSummaries.isEmpty()) {
			usageDataService.call(
					new RemoteCallback<Queue<UsageEventSummary>>() {
						@Override
						public void callback(Queue<UsageEventSummary> events) {
	...
	public void searchEvents(String text) {
		if (text.equals("")) {
			if (allUsageEventSummaries != null) {
				...
				dataProvider
						.setList(Lists.newArrayList(allUsageEventSummaries));
				dataProvider.refresh();
			}
		} else {
			if (allUsageEventSummaries != null) {
				List<UsageEventSummary> tasks = Lists
						.newArrayList(allUsageEventSummaries);
				List<UsageEventSummary> filteredTasksSimple = Lists
						.newArrayList();
				for (UsageEventSummary ts : tasks) {
					if (ts.getComponent().toLowerCase()
							.contains(text.toLowerCase())
							|| ts.getDescription().toLowerCase()
									.contains(text.toLowerCase())) {
				...
				dataProvider.refresh();
				auditEvent("Search", "Search words: " + text, identity.getName(), 
						EventsUsageData.UDC_SEARCH, StatusUsageEvent.SUCCESS, LevelsUsageEvent.INFO);
			}
		}
	...
	public void filterEventsByType(EventTypes eventType) {
		...
		}).readUsagesByFilter(eventType, identity.getName());
	}
	...
	public void exportToTxt() {
		PlaceRequest placeRequestImpl = new DefaultPlaceRequest(
				"Export Usage Data");
		placeRequestImpl.addParameter("type", view.getEventTypesList()
				.getValue());
	 ...
	 */
	public void auditEvent(String key, String description, String user, EventsUsageData usageEvent, StatusUsageEvent status, LevelsUsageEvent level) {
